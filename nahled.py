...
# (kod kreditora, nazev, identita, ico, dic, zeme, mesto, web, nace, objem,
#  kod kategorie, co dodava, jistota, obory, poznamky)
UKAZKA = [
    ("D0001", "Alza.cz a.s.", IDENT_REJSTRIK, "27082440", "CZ27082440", "CZ",
     "Praha", "https://www.alza.cz", ["47910"], "1 250 000",
    ...
    ("D0002", "GABEN, spol. s r. o.", IDENT_REJSTRIK, "19012021", "CZ19012021", "CZ",
     "Ostrava", "https://www.gaben.cz", ["46520", "62090"], "89 000",
...
VYPLNENO = {
    "D0001": (0, 0, "J. Dvořák (nákup)", -120, 610, ""),
    "D0002": (2, 1, "M. Horák (výroba)", -60, 300, "Přístup do skladového systému přes API."),
    "D0003": (0, 1, "M. Horák (výroba)", -200, 530, ""),
    "D0004": (2, 0, "P. Nový (IT)", -30, 335, "DPA podepsáno, telemetrie mimo EU."),
    "D0005": (2, 2, "P. Nový (IT)", -30, 335,
              "Provoz webu stojí a padá s nimi, migrace by trvala týdny."),
    "D0006": (2, 1, "P. Nový (IT)", -400, -30, "Přezkoumání propadlo, řeší se."),
    "D0007": (0, 1, "", None, None, ""),
    "D0008": (1, 0, "L. Marek (provoz)", -90, 275, ""),
    "D0009": (1, 0, "", None, None, "Vstup do prostor mimo pracovní dobu."),
    "D0010": (2, 1, "K. Beránková (právní)", -150, 580, ""),
    "D0011": (3, 2, "", None, None,
              "Vzdálená správa ERP včetně přístupu do databáze – chybí vlastník."),
    "D0012": (1, 0, "L. Marek (provoz)", -220, 500, ""),
    "D0013": (0, 0, "", None, None, ""),
...
def _firmy():
    out = []
    for (kod, nazev, identita, ico, dic, zeme, mesto, web, nace, objem,
         _k, _c, _j, obory, poznamky) in UKAZKA:
        f = Firma(vstup_kod=kod, vstup_nazev=nazev, vstup_ico=ico,
                  vstup_dic=dic, vstup_zeme=zeme, vstup_objem=objem)
...
def _odpovedi():
    return {i: {"kod": r[10], "popis": r[11], "jistota": r[12]}
            for i, r in enumerate(UKAZKA, 1) if r[10]}
